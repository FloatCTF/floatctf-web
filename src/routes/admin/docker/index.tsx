...
function RouteComponent() {
  ...
  const columns = [
    { accessorKey: "id", header: "Container ID", field: "id", rowHeader: true },
  ...
  const stopMutation = useMutation({
    mutationFn: (container_id: string) => adminApi.docker.stopContainer(container_id),
    onSuccess: () => {
  ...
  const startMutation = useMutation({
    mutationFn: (container_id: string) => adminApi.docker.startContainer(container_id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["Containers"] });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (container_id: string) => adminApi.docker.deleteContainer(container_id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["Containers"] });
    },
  });
