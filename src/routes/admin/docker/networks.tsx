...
function RouteComponent() {
  ...
  const columns = [
    { accessorKey: "id", header: "Network ID", field: "id", rowHeader: true },
  ...
  const createMutation = useMutation({
    mutationFn: (network: { name: string; subnet: string; gateway: string; driver?: string }) =>
      adminApi.docker.createNetwork(network),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["Networks"] });
      setIsOpen(false);
      setNewNetwork({ name: "", subnet: "", gateway: "", driver: "bridge" });
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (network_id: string) => adminApi.docker.deleteNetwork(network_id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["Networks"] });
    },
  });
        ...
        >
          <Stack direction="vertical" gap="spacious">
            ...
            <Button
              ...
              disabled={!newNetwork.name || !newNetwork.subnet || !newNetwork.gateway}
            >
              Create
            </Button>
          </Stack>
